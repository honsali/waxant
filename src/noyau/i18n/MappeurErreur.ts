import { IErreurSimple, IInfoActionEchouee, IMessageErreur } from '../message/DomaineMessage';
...
const trouverLibelleErreur = ({ code, args, libelle }: IErreurSimple, mapErreur: Record<string, string>): string => {
    if (args?.length) {
...
const get = ({ code, listeErreurSimple, erreur }: IInfoActionEchouee, mapErreur: Record<string, string>): IMessageErreur => {
    ...
    if (e) {
        ...
        if (listeErreurSimple?.length) {
            messageErreur.listeErreur = listeErreurSimple.map((err) => trouverLibelleErreur(err, mapErreur));
        }
        if (erreur) {
