...
const Etat = ({ modele = null, nombreColonne = 2, afficherLesVide = true, children }) => {
    ...
    const getTexte = useCallback((m, cprops, propNom): any => {
        ...
        if (cprops.texte) {
            return cprops.texte;
        } else if (cprops[propNom] && _.nonVide(m)) {
            const a = _.get(m, cprops[propNom]);
            return _.nonNul(a) ? a : dflt;
        } else {
            return dflt;
        }
    }, []);
    ...
    useEffect(() => {
        ...
        if (_.nonVide(modele)) {
            React.Children.forEach(children, (c) => {
                ...
                if ((afficherLesVide || text !== '-') && (_.estNul(c.props.invisible) || !c.props.invisible)) {
